<?php
  ...
  if (isset($_GET['added'])) {
    ...
    printf (_('-- Your vacation message was too long: %1$d characters. It has been truncated at %2$d.'), html_escape($_GET['uservacationtolong'], $max_vacation_length));
  } // Now some more general errors on account updates
  else if (isset($_GET['badaliaspass'])) {
    printf (_("-- Account %s could not be added. Your passwords are blank or do not match."), html_escape($_GET['badaliaspass']));
  } else if (isset($_GET['badname'])) {
    printf (_("-- %s contains invalid characters."), html_escape($_GET['badname']));
  } else if (isset($_GET['badpass'])) {
    printf (_("-- Account %s not added/updated. Passwords were blank, did not match, or contained illegal characters: ' \" ` or ;"), html_escape($_GET['badpass']));
  } else if (isset($_GET['weakpass'])) {
    printf (_("-- Password too weak. Must have upper/lower case, numbers, special characters and be at least 8 characters long!"), html_escape($_GET['weakpass']));
    ...
    printf (_("-- Domain %s could not be added. There was a problem adding the domain."), html_escape($_GET['failaddeddomerr']));
    ...
    printf (_("-- The maximum messages size you specified was too high. The maximum size you can specify is: %s KB."), html_escape($_GET['maxmsgsizehigh']));
    ...
    printf (_("-- Spamassassin must be enabled for Spam Box to be enabled."), html_escape($_GET['spamboxerr']));
  } else if (isset($_GET['spamreporterr'])) {
    printf (_("-- Spamassassin and Spam Box must be enabled for Spam Report to be enabled."), html_escape($_GET['spamreporterr']));
