...
<?php
/**
 * $Horde: groupware/scripts/setup.php,v 1.34.2.3 2009/08/12 17:51:09 jan Exp $
 *
...
function config_admin()
{
    ...
    if ($GLOBALS['bundle'] == 'webmail') {
        $admin_user = $cli->prompt('Specify an existing mail user who you want to give administrator permissions (optional):');
    } else {
...
function update()
{
    ...
    $old_dir = $cli->prompt('Specify the directory of the old Horde Groupware Webmail Edition version that you would like to update:');
    ...
    foreach ($update_files as $update_file) {
        ...
        if (version_compare($new_version, '1.2.4', '>') ||
